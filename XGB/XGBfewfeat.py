...
HeldOutAdd = OUTPUTFOLDER+'/HeldOut'
...
for FeaturePick in range(1,1+MaxFeaturePick):

    print('Only use '+str(FeaturePick)+' features...')
    FeatTable = pd.read_csv(RootOUTPUTFOLDER+'/FeatureSelection/FEATTABLE-SHAP.csv')
    ImportantKMERS = ['Antibiotic','MIC'] + list(FeatTable.iloc[:,1])[:FeaturePick]
    df = dfmain[ImportantKMERS]
    
    OUTPUTFOLDER = RootOUTPUTFOLDER+'/FeatureSelection/'+str(FeaturePick)
    ...
    for i, (train_index, test_index) in enumerate(skf.split(X, CLASS)):
        ...
        Train_predict = model.predict(d_train_test, ntree_limit=model.best_ntree_limit);
        Trainacc = accuracy_dilute(Train_predict,Y_train_t);
...
# Random features
os.system('mkdir '+RootOUTPUTFOLDER+'/FeatureSelection/Random')
for FeaturePick in range(1,1+MaxFeaturePick):

    print('RANDOM: Use '+str(FeaturePick)+' features...')
    FeatTable = pd.read_csv(RootOUTPUTFOLDER+'/FeatureSelection/FEATTABLE-SHAP.csv')
    ImportantKMERS = ['Antibiotic','MIC'] + random.sample(list(FeatTable.iloc[:,1]),FeaturePick)
    df = dfmain[ImportantKMERS]
    
    OUTPUTFOLDER = RootOUTPUTFOLDER+'/FeatureSelection/Random/'+str(FeaturePick)
    os.system('mkdir '+OUTPUTFOLDER)
...
NfeatVec = range(1,1+MaxFeaturePick)

cwd = RootOUTPUTFOLDER+'/FeatureSelection'
ACCmean = []
ACCstd = []
for ii in NfeatVec:
    ACC = pd.read_csv(cwd+'/'+str(ii)+'/ACCU.txt',header = None)
    ACCmean.append(np.mean(ACC))
    ACCstd.append(np.std(ACC))
    

plt.plot(NfeatVec,ACCmean,color='b')


ACC = pd.read_csv(RootOUTPUTFOLDER+'/ACCU.txt',header = None)
MeanACCall = np.mean(ACC)
plt.hlines(MeanACCall,1,MaxFeaturePick,color='r')

plt.grid(True)
plt.xlabel('Number of features')
plt.ylabel('Accuracy')
plt.legend(['Using selected features','Using all features'])

plt.tight_layout()
plt.savefig(cwd+'/FewFeat.png')
plt.savefig(cwd+'/FewFeat.pdf')


BestNFeat =  np.argmax(ACCmean) + 1 #Add 1 due to zeros indexing 
f2 = open(cwd+"/BestNumberOfFeatures.txt", "w")
f2.write(str(BestNFeat)+"\n")
f2.close()
...
##############################################################################
#Final Evaluation:
OUTPUTFOLDER = RootOUTPUTFOLDER+'/FeatureSelection/FinalEval'
os.system('mkdir '+OUTPUTFOLDER)

FeaturePick = BestNFeat

print('Final evaluation. Only use '+str(FeaturePick)+' features...')
FeatTable = pd.read_csv(RootOUTPUTFOLDER+'/FeatureSelection/FEATTABLE-SHAP.csv')
ImportantKMERS = ['Antibiotic','MIC'] + list(FeatTable.iloc[:,1])[:FeaturePick]
df = dfmain[ImportantKMERS]
...
#Add final evaluation to the plot
FIG = plt.figure()
NfeatVec = range(1,1+MaxFeaturePick)
...
cwdR = RootOUTPUTFOLDER+'/FeatureSelection/Random'
ACCmeanR = []
ACCstdR = []
for ii in NfeatVec:
    ACCR = pd.read_csv(cwd+'/'+str(ii)+'/ACCU.txt',header = None)
    ACCmeanR.append(np.mean(ACC))
    ACCstdR.append(np.std(ACC))
    

plt.plot(NfeatVec,ACCmean,marker='x',color='b')
plt.plot(NfeatVec,ACCmeanR,marker='x',color='y')

ACC = pd.read_csv(RootOUTPUTFOLDER+'/ACCU.txt',header = None)
MeanACCall = np.mean(ACC)
plt.hlines(MeanACCall,1,MaxFeaturePick,color='r')

plt.plot(BestNFeat,FINALACC,'x',color='g')

plt.grid(True)
plt.xlabel('Number of features')
plt.ylabel('Accuracy')
plt.legend(['Mean CV accuracy using selected features',
            'Mean CV accuracy using random features',
            'Final Evaluation Accuracy on Held-out set',
            'Mean CV accuracy using all features'])

plt.tight_layout()
plt.savefig(cwd+'/FINALR.png')
plt.savefig(cwd+'/FINALR.pdf')
pickle.dump(FIG,open(cwd+'/FINALR.pickle','wb'))
